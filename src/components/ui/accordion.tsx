...
>( ( { className, ...props }, ref ) => (
    <AccordionPrimitive.Item
        ref={ref}
        className={cn( "border-b border-softNeutral-200", className )} // Adding softNeutral border
        {...props}
    />
) );
...
>( ( { className, children, ...props }, ref ) => (
    <AccordionPrimitive.Header className="flex">
        ...
        >
            {children}
            <ChevronDown className="h-4 w-4 shrink-0 text-mutedGreen-500 transition-transform duration-200" />
        </AccordionPrimitive.Trigger>
